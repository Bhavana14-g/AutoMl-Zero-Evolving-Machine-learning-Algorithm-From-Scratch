...
// Scalar and Vector are trivially destructible.
    ...
    TEST(FillTasksTest, WorksCorrectly) {
        ...
        EXPECT_LT(
                (tasks[0]->train_labels_[478].getVector() -
                 expected_task_0.train_labels_[478].getVector()).norm(),
                kDataTolerance);
        EXPECT_LT(
                (tasks[1]->train_labels_[478].getVector() -
                 expected_task_1.train_labels_[478].getVector()).norm(),
                kDataTolerance);
        EXPECT_LT(
                (tasks[0]->valid_labels_[94].getVector() -
                 expected_task_0.valid_labels_[94].getVector()).norm(),
                kDataTolerance);
        EXPECT_LT(
                (tasks[1]->valid_labels_[94].getVector() -
                 expected_task_1.valid_labels_[94].getVector()).norm(),
                kDataTolerance);
    ...
    TEST(FillTaskWithZerosTest, WorksCorrectly) {
        ...
        for (const Label<4> &label : dataset.train_labels_) {
            EXPECT_FLOAT_EQ(label.getVector().norm(), 0.0);
        }
    ...
    TEST(FillTaskWithOnesTest, WorksCorrectly) {
        ...
        for (const Label<4> &label : dataset.train_labels_) {
            EXPECT_LE((label.getVector() - Vector<4>::Ones(4, 1)).norm(), kDataTolerance);
        }
        for (const Vector<4> &feature : dataset.valid_features_) {
            EXPECT_TRUE(feature.isApprox(kOnesVector));
        }
    ...
    TEST(FillTaskWithIncrementingIntegersTest, WorksCorrectly) {
        ...
        EXPECT_LE(dataset.train_labels_[0].getVector().norm(), kDataTolerance);
        EXPECT_TRUE(
                dataset.train_labels_[kNumTrainExamples - 1].getVector().isApprox(
                        kOnesVector * static_cast<double>(kNumTrainExamples - 1)));
        ...
        EXPECT_TRUE(dataset.valid_labels_[0].getVector().isApprox(kZeroVector));
        EXPECT_TRUE(
                dataset.valid_labels_[kNumValidExamples - 1].getVector().isApprox(
                        kOnesVector * static_cast<double>(kNumValidExamples - 1)));
    }
