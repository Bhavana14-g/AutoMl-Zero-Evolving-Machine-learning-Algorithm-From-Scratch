...
namespace automl_zero
{
    ...
    // s1 = v3[5]
    template <FeatureIndexT F>
    inline void ExecuteScalarVectorAtIndexSetOp(
        const Instruction &instruction, RandomGenerator *rand_gen,
        Memory<F> *memory)
    {
        const FeatureIndexT index = FloatToIndex(instruction.GetFloatData0(), F);     // 5
        memory->scalar_[instruction.out_] = memory->vector_[instruction.in1_](index); // v3[5]
    }
    ...
    struct SortedLossAccumulator
    {
        ...
        static double getError(const Vector<F> &numbers)
        {
            int numErrors = 0;
            ...
            return (double)numErrors / (double)(numbers.size() - 1);
        }
    };
